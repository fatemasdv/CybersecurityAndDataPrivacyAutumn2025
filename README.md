# Cybersecurity and Data Privacy – Autumn 2025

This repository contains my work and logbook for the **Cybersecurity and Data Privacy (Autumn 2025)** course.

## 📘 Description
The purpose of this repository is to document my learning progress throughout the course, including:
- Weekly topics and lectures
- Assignments and exercises
- Personal reflections and outcomes

## 🧾 Files in this repository
- **logbook.md** → My learning log (daily/weekly updates)
- **README.md** → This description file

## 🧠 Course Information
- **Course name:** Cybersecurity and Data Privacy  
- **Semester:** Autumn 2025  
- **Student:** *Your Name (Fatema Akter)*  
- **Instructor:** *Vheikkiemi*  

## ✅ Progress summary
- Repository created on **2025-11-09**
- Logbook initialized and first entries added
- Future entries will be added weekly
